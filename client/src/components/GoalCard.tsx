import { DesplegableDown, DesplegableUp } from "../assets/svgs"
...
export function GoalCard({goal}: {goal: Goal}){
  ...
  { id: "t3", linkedGoalId: goal.id, title: "Hacer ejercicios", startDate: "2026-06-20", isCompleted: false }
    ...
    return (
        <div onClick={()=> setIsExpanded(!isExpanded)} className={isExpanded ? "bg-[#122131] pt-5  mb-2.5 h-auto flex flex-row justify-between rounded-xl border-[#3C4A46] border ": "bg-[#122131] pt-5  mb-2.5 h-auto flex flex-row justify-between rounded-t-xl border-[#3C4A46] border-t border-r border-l "}>
            ...
            <div className="pr-4">
                <span>{isExpanded ? <DesplegableUp/>: <DesplegableDown/>}</span>
            </div>
        </div>
    )
}
